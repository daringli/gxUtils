...
def get_Q(d, tf_fallback = None):
    ...
    if not np.isnan(tf):
        Qi, t = get_Qi_t_nc(d)
        i_tf = np.argmax(t>tf)
        autocorrelation(t[i_tf:],Qi[i_tf:], autocor_threshold=autocor_threshold)
        ...
        Qavg = np.trapz(Qi[i_tf:], t[i_tf:])/T
        print("Qavg, Qmean")
        print(Qavg)
        print(np.mean(Qi))
        Qvar = np.trapz((Qi[i_tf:] - Qavg)**2, t[i_tf:])/T
...
if __name__ == "__main__":
    ...
    for i,d in enumerate(ds):    
        tf = get_end_of_transient_time(d, twindow=twindow, mthreshold=mthreshold, sigmathreshold=sigmathreshold, plot = True, color=cmap(i), axes=axes)
        if not np.isnan(tf):
            #print(d)
            #print(tf)
            legend.append(d)
